...
class SquareClient:
    ...
    def search_customers(self, email: str = None, phone: str = None) -> List[Customer]:
        """Search customers by email or phone using Square Search API."""
        filters = {}
        if email:
            filters["email_address"] = {"exact": email.lower()}
        if phone:
            clean = ''.join(c for c in phone if c.isdigit())
            if len(clean) >= 10:
                filters["phone_number"] = {"exact": f"+1{clean[-10:]}"}
        ...
        try:
            data = self._post("/v2/customers/search", payload)
